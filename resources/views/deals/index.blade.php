@extends('layouts.app', ['title' => 'InnovaCRM - Deals'])

@section('content')
    <x-breadcrumb :items="[['label' => 'Dashboard', 'url' => route('dashboard')], ['label' => 'Deals']]" />

    <div class="card border-0 shadow-sm rounded-4 overflow-hidden bg-body">
        <div class="card-header border-0 bg-body p-3">
            <x-page-header title="Deals" icon="fa-solid fa-gem" :cardHeader="false" class="mb-4">
                <x-slot:actions>
                    @can('deals.create')
                        <x-button.primary href="{{ route('deals.create') }}" icon="fa-solid fa-plus fs-sm" label="Add Deal" />
                    @endcan
                </x-slot:actions>
            </x-page-header>
            ...
            <div class="filter-controls-wrapper d-flex flex-column flex-md-row align-items-stretch align-items-md-center justify-content-between gap-2">
                ...
                <div class="d-flex flex-wrap flex-sm-nowrap align-items-center gap-2 order-1 order-md-2 ms-md-auto">
                    <div class="search-input-box px-3 py-1 d-flex align-items-center flex-grow-1 flex-sm-grow-0">
                        ...
                        <input type="text" id="searchInput" class="form-control border-0 bg-transparent shadow-none p-1 fs-sm w-100" placeholder="Search deals...">
                    </div>
                    ...
                    <div class="filter-item-half">
                        <button class="btn btn-filter-action shadow-none w-100 d-flex align-items-center gap-2 justify-content-center text-nowrap" id="btnExport" title="Export Deals">
        ...
        <div class="d-none d-lg-block">
            <x-data-table id="dealsTable" tableBodyId="dealsTableBody" :showCheckboxColumn="true" :headers="[
                'DEAL TITLE',
                'VALUE',
                'STAGE',
                'COMPANY / CONTACT',
        ...
        <div class="d-lg-none border-top">
            <div id="dealsMobileCardList" class="d-flex flex-column bg-body"></div>
...
@push('scripts')
    <script>
        window.userPermissions = {
            canEdit: @json(auth()->user()->can('deals.edit')),
            canDelete: @json(auth()->user()->can('deals.delete'))
        };
    </script>
